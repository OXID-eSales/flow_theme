...
$(function() {
  ...
  const $oBasketList = $('#basket_list');
  const $oToTop = $('#jumptotop');
  const $oRecommendations = $('#econdaRecommendations');
  const $oChangeEmail = $('input.oxValidate_enterPass');
  const $oSearchInput = $('#searchParam');
  const iHeaderWrapperHeight = parseInt($oHeader.height());
  const blIsCheckout = $oBody.hasClass('is-checkout');
  const $oContentWrapper = $('#content');
  ...
  if ($oRecommendations.length) {
    $.get(
        window.sBaseUrl + 'cl=tpl&tpl=ajax_econda_recommendations.tpl&actcl=' +
            sActCl + '', function(oData, sStatus, oXhr) {
  ...
  const $oCategoryBox = $oSidebar.find('.box.categorytree');
  if ($oCategoryBox.length) {
    $oCategoryBox.find('.toggleTree').on('click touch', function() {
      const $this = $(this);
      const $oCategoryTreeBox = $this.parents('.categorytree').
          find('.categoryBox');
  ...
  /* *********************************
             ...
             * *********************************/
  const $oSelectionLists = $('.listDetails .selectorsBox');
  ...
  function reRenderMainNav() {
    const $oMainNav = $('#mainnav').find('.navbar-collapse');
    const $oNavList = $('#navigation');
    let $oMoreLinks = $oNavList.find('.moreLinks');
    ...
    if ($oMoreLinks.length) {
      ...
      const oMoreLinkElem = document.createElement('li');
      const oMoreLinkAElem = document.createElement('a');
      const oMoreLinkUlElem = document.createElement('ul');

      oMoreLinkElem.className = 'dropdown moreLinks';

      oMoreLinkAElem.className = 'dropdown-toggle';
      oMoreLinkAElem.innerHTML = oFlow.i18n.DD_NAVIGATION_MORE +
    ...
    let iMainNavWidth = $oMainNav.width();
    const $oNavItems = $oNavList.find('> li').not('.moreLinks');
    let iNavItemsWidth = 0;
    const aMoreLinkElems = [];
  ...
  if ($oBasketList.length) {
    $('#basket_form').on('submit', function(e) {
      const $this = $(this);
      const $oHiddenXs = $this.find('.hidden-xs');
      const $oVisibleXs = $this.find('.visible-xs');
    ...
    $oBasketList.find('.toggle-actions').click(function(e) {
      e.preventDefault();
      const $this = $(this);
      const $oToggable = $this.parents('li').first().find('.row.toggable');

      $oToggable.slideToggle(150, function() {
        $this.find('i').
            attr('class', ($oToggable.is(':visible') ?
                            'fa fa-chevron-up' :
                            'fa fa-chevron-down'));
      },
      );
  ...
  $('.basketItemDesc .selectorsBox .dropdown-menu li a', $oContentWrapper).
      click(function(e) {
        e.preventDefault();
        const $this = $(this);
  ...
  if ($oChangeEmail.length) {
    const sOldMail = $oChangeEmail.val();
    const $oPasswordElem = $('.oxValidate_pwd');
  ...
  $('#private-sales-login input.agb-check').click(function() {
    if ((this).checked) {
      ...
      $('#private-sales-login button.submitButton').
