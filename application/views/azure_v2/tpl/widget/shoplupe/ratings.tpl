[{block name="dd_widget_sidebar_shoplupe_ratings"}]
    <div class="box well well-sm">
        <section>
            <div class="page-header h3">[{oxmultilang ident="WIDGET_SHOPLUPE"}]</div>
            [{block name="dd_widget_sidebar_shoplupe_ratings_inner"}]
                <div class="content">
                    <p class="shoplupe">
                        <a href="http://www.shoplupe.com" target="_blank" title="[{oxmultilang ident="WIDGET_SHOPLUPE_RATINGS_INFO_URL_TITLE"}]">
                            <img src="[{$oViewConf->getImageUrl('shoplupe.jpg')}]" alt="[{oxmultilang ident="WIDGET_SHOPLUPE_RATINGS_ALT"}]" width="119" height="133">
                        </a>
                    </p>
                </div>
            [{/block}]
        </section>
    </div>
[{/block}]
