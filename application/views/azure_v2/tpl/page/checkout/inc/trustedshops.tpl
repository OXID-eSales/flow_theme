        ...
        <div id="tsBox">
            ...
            <div>
                ...
                [{if count($aTsProtections) > 1}]
                    <select name="stsprotection">
                        [{foreach from=$aTsProtections item=oTsProduct}]
                            [{if $oView->isPaymentVatSplitted()}]
                                <option value="[{$oTsProduct->getTsId()}]" [{if $oView->getCheckedTsProductId() == $oTsProduct->getTsId()}]SELECTED[{/if}]>[{oxmultilang ident="TRUSTED_SHOP_PROTECTION_FROM"}] [{$oTsProduct->getAmount()}] [{$currency->sign}] ([{$oTsProduct->getFNettoPrice()}] [{$currency->sign}] [{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_PLUSTAX1"}] [{$oTsProduct->getFVatValue()}] [{$currency->sign}] ) </option>
                            [{else}]
                    ...
                    [{if $oView->isPaymentVatSplitted()}]
                        <input type="hidden" name="stsprotection" value="[{$oTsProduct->getTsId()}]">[{oxmultilang ident="TRUSTED_SHOP_PROTECTION_FROM"}] [{$oTsProduct->getAmount()}] [{$currency->sign}] ([{$oTsProduct->getFNettoPrice()}] [{$currency->sign}] [{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_PLUSTAX1"}] [{$oTsProduct->getFVatValue()}] [{$currency->sign}])
                    [{else}]
